...
#ifndef UAPP_AD7771_H_
#define UAPP_AD7771_H_

/***********************SPI 4 interface **********************************
P10： AD7771 DCLK = 4.096Mhz
P10： SPI时序图 模式1
P31： 单位转换 LSB = 2 x Vref /Gain /2^23
P33： 噪声
P34： 输入模式 1.True Differential		2. Pseudo differential	3. Single-ended
			AD7779提供 （AVDD + AVSS）/2共模电压引脚
P36： REF改为外接ADE441可以降低噪声
P37： 缓冲预充模式，有更小的内部噪声
			MCLK在高分辨率模式 = Cry / 4
P41： 相对增益0x555555 gain of 1
P43： 内部参考选择
P44： power down	LDO旁路参数
P45： SPI命令格式
P46： SPI的SAR检测模式  ADC Mode  SPI软件复位
P47-48：  多种SPI自检测功能
P51： ADC data数据格式 Header + 24bit ADC data
P51： 采样率的设计和计算！！！  设置寄存器SRC_N_LSB/MSB  ORD = （HRmode）2048 / 1（Khz） = 2048
...
// 设置寄存器：AD7779_REG_CHx_CONFIG
#define AD7779_CH_GAIN(x)											(((x) & 0x3) << 6)				//设置增益
#define AD7779_CH_REF_MONITOR									(1 << 5)									//设置通道用于参考监听
#define AD7779_CH_RX													(1 << 4)									//通道表Mux RX模式

// AD7779_REG_CH_DISABLE
#define AD7779_CH_DISABLE(x)									(1 << (x))								//关闭对应通道
...
// user write function
void Read_RegList_init();		//填充寄存器列表
...
void eCon_ad7779_init();		//AD777x初始化
